...
def inference(vc_model_path, src_wav_path, tgt_wav_path, output_path, codebooksize):
    ...
    # A -> Global
    count_map_A_to_G = torch.load(f'mappings/{codebooksize}/count_matrix_{src_spk}_to_global.pt', map_location=device)
    count_map_A_to_G = patch_unseen_mapping(count_map_A_to_G, quantizer_src) # 패치!
    row_sums = count_map_A_to_G.sum(dim=1, keepdim=True) + 1e-8
    prob_map_A_to_G = count_map_A_to_G.float() / row_sums
    ...
    count_map_G_to_B = patch_unseen_mapping(count_map_G_to_B, quantizer_global) # 패치! (입력이 Global임)
    ...
    print("Calculating the final A -> B map sequentially to save memory...")

    a_vocab_size = prob_map_A_to_G.shape[0]
    final_map_list = []

    # A의 각 인덱스에 대해 반복
    for i in tqdm(range(a_vocab_size), desc="Mapping A to B"):
        #  prob_map을 사용하여 정확한 확률(경로 점수) 계산
        a_to_g_row = prob_map_A_to_G[i] # (G_size,)
        
        # (G_size, 1) * (G_size, B_size) -> (G_size, B_size)
        scores_for_A_i = a_to_g_row.unsqueeze(1) * prob_map_G_to_B
        
        # G를 거치는 경로 중 가장 점수(확률)가 높은 경로를 B 인덱스별로 찾음 (B_size,)
        max_scores_per_b, _ = torch.max(scores_for_A_i, dim=0)

        # 가장 점수(확률)가 높은 최종 B 인덱스 하나를 선택
        best_b_index = torch.argmax(max_scores_per_b)
        
        final_map_list.append(best_b_index)

    # 리스트를 텐서로 변환 (Vocab_A 크기의 1D 텐서)
    final_map_A_to_B = torch.stack(final_map_list).to(device)
    print("Final map calculation complete.")

    # 생성된 맵을 이용해 인덱스 변환 및 벡터 조회
    # Source VQ에서 Index 가져오기
    indices_src = quantizer_src.get_code_indices(mean) # (B, T)

    # 최종 맵을 참조하여 B의 인덱스로 다이렉트 변환
