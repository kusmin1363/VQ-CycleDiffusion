...
def inference(vc_model_path, src_wav_path, tgt_wav_path, output_path, codebooksize):
    ...
    print(codebooksize)
    # VQ Load
    quantizer_global = VectorQuantizer(embedding_dim=params.embedding_dim, num_embeddings=codebooksize, commitment_cost=0.25).to(device)
    quantizer_global.load_state_dict(torch.load(f'log/codebook_stock_255_exclude/global/codebook_stock_{codebooksize}.pt', map_location=device))
    quantizer_global.eval()
    ...
    # Map Load + Transpose + Row-sum + Normalization
    count_map_B_to_G = torch.load(f'mappings/{codebooksize}/count_matrix_{tgt_spk}_to_global.pt', map_location=device)
    count_map_G_to_B = count_map_B_to_G.T

    try:
        # ==========================================
        # [맵 수리 패치] Unseen Global Index를 Nearest Seen Index로 대체
        row_sums = count_map_G_to_B.sum(dim=1)
        ...
        if unseen_mask.any():
            ...
            # 1. Global Codebook의 실제 임베딩 벡터 가져오기
            global_embeds = quantizer_global.embeddings.weight.data # (G_Vocab, D)
            
            # 2. Seen / Unseen 벡터 분리
            seen_indices = torch.nonzero(seen_mask).squeeze()
            seen_embeds = global_embeds[seen_mask]      # (Num_Seen, D)
            unseen_embeds = global_embeds[unseen_mask]  # (Num_Unseen, D)
            
            # 3. Unseen 벡터들과 Seen 벡터들 사이의 L2 거리(유사도) 계산
            # torch.cdist는 두 텐서 간의 모든 쌍의 거리를 계산해줍니다.
            dist = torch.cdist(unseen_embeds, seen_embeds) # (Num_Unseen, Num_Seen)
            
            # 4. 각 Unseen마다 가장 거리가 짧은(가까운) Seen 인덱스 찾기
            nearest_seen_idx_in_subset = dist.argmin(dim=1)
            nearest_seen_idx_global = seen_indices[nearest_seen_idx_in_subset]
            
            # 5. 매핑 테이블 덮어쓰기 (가장 가까운 특징의 타겟 매핑 카운트를 그대로 복사)
            count_map_G_to_B[unseen_mask] = count_map_G_to_B[nearest_seen_idx_global]
            # ==========================================
    except Exception as e:
            print(f"맵 수리 패치 중 에러 발생: {e}")
            
    col_sums = count_map_G_to_B.sum(dim=1, keepdim=True) + 1e-8
    prob_map_G_to_B = count_map_G_to_B.float() / col_sums

    map_G_to_B = count_map_G_to_B
    ...
    # Step 1: Source Audio -> Global Index
    # Source VQ 없이, 오디오 특징(mean)을 바로 Global Codebook에 매칭
    indices_global = quantizer_global.get_code_indices(mean) # (B, T)
    
    # Flatten: (B*T)
    flat_indices_global = indices_global.flatten()

    # Step 2: Global Index -> Target Index (Argmax)
    # 현재 Global Index에 해당하는 Target 매핑 행(Row)들을 가져옴
    # shape: (B*T, Target_Vocab_Size)
    current_rows = map_G_to_B[flat_indices_global]

    # [핵심 변경] Weighted Sum(@) 대신 가장 값이 큰 인덱스 하나 선택 (Argmax)
    indices_tgt = torch.argmax(current_rows, dim=1) # (B*T)

    # Step 3: Target Vector Lookup
    # Target Codebook에서 인덱스에 해당하는 벡터 1개씩 쏙쏙 뽑아옴
