...
def inference(vc_model_path, src_wav_path, tgt_wav_path, output_path, codebooksize):
    ...
    try:
        ...
        print(f"오류: G->B 다이렉트 맵 파일이 없습니다!")
        return
    ...
    # Step 1: Global
    # Global VQ에서 Index 가져오기
    indices_global = quantizer_global.get_code_indices(mean)
    flat_indices = indices_global.flatten()

    current_probs = prob_map_G_to_B[flat_indices]
    ...
    # probs_G_to_B = prob_map_G_to_B[indices_global.flatten()]

    # # Step 2: Global -> B
    # # Weighted Sum: Target VQ의 Codebook으로 가중합
    # target_codewords = quantizer_tgt.embeddings.weight
    # # (B*T, N_target) @ (N_target, D) -> (B*T, D)
    # final_vectors = probs_G_to_B @ target_codewords
