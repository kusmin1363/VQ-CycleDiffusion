...
def inference(vc_model_path, src_wav_path, tgt_wav_path, output_path, codebooksize):
    ...
    # 1. Map Load & Unseen Index Patching
    # A -> Global
    count_map_A_to_G = torch.load(f'mappings/{codebooksize}/count_matrix_{src_spk}_to_global.pt', map_location=device)
    count_map_A_to_G = patch_unseen_mapping(count_map_A_to_G, quantizer_src) 
    row_sums_A_to_G = count_map_A_to_G.sum(dim=1, keepdim=True) + 1e-8
    prob_map_A_to_G = count_map_A_to_G.float() / row_sums_A_to_G
    ...
    count_map_G_to_B = patch_unseen_mapping(count_map_G_to_B, quantizer_global)
    col_sums = count_map_G_to_B.sum(dim=1, keepdim=True) + 1e-8
    prob_map_G_to_B = count_map_G_to_B.float() / col_sums

    # 2. 미리 A -> B 통합 확률 맵 계산
    # (Vocab_A, Vocab_G) @ (Vocab_G, Vocab_B) -> (Vocab_A, Vocab_B)
    prob_map_A_to_B_combined = prob_map_A_to_G @ prob_map_G_to_B
    ...
    indices_src = quantizer_src.get_code_indices(mean) 
    
    # 미리 곱해둔 통합 맵에서 바로 타겟 확률 뽑아오기
    probs_final_B = prob_map_A_to_B_combined[indices_src.flatten()]
    
    # Temperature를 적용한 Softmax 연산
    tau = 0.5
    logits = torch.log(probs_final_B + 1e-9)
    probs_final_B_soft = torch.nn.functional.softmax(logits/tau, dim=1)

    # 4. Weighted Sum 
    # Target VQ의 Codebook으로 가중합
    target_codewords = quantizer_tgt.embeddings.weight
    # (B*T, N_target) @ (N_target, D) -> (B*T, D)
    final_vectors = probs_final_B_soft @ target_codewords
    B, D, T = mean.shape
