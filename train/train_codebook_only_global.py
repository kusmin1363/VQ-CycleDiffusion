...
if __name__ == "__main__":
    ...
    for learning_rate in learning_rates :
        log_dir = f'log/Codebook_only_train/log_codebook_{codebooksize}_{learning_rate}'

        os.makedirs(log_dir, exist_ok=True)

        msg = f"""
        Training Quantizer only
        ...
        for epoch in range(epochs):
            ...
            for batch in pbar:
                ...
                # 3. Decoder (Frozen, 10-step prediction)
                with torch.no_grad():
                    mel_pred = model.decoder(
                        z_q,                # Input (Condition): Quantized Latent
                        x_mask,             # Mask
                        mean=z_q,           # mean: 보통 Encoder Output을 의미 (여기선 Quantized된 값 사용)
                        ref=mel_source,     # ref: 스타일 참고용 (복원이므로 자기 자신 mel)
                        ref_mask=x_mask,    # ref_mask: 자기 자신 mask
                        mean_ref=z,         # mean_ref: 참고 오디오의 Latent (Quantization 전의 z 사용)
                        c=c,                # c: 화자 임베딩
                        n_timesteps=10, 
                        mode='ml'
                    )
            ...
            log_msg = (f"Epoch {epoch+1}: "
                        f"Train[R={avg_train_recon:.4f}, VQ={avg_train_vq:.4f}] | "
                        f"Valid[R={avg_val_recon:.4f}, VQ={avg_val_vq:.4f}]")
            print(log_msg)
            ...
            if (epoch + 1) % 10 == 0:
                torch.save(quantizer.state_dict(), f"{log_dir}/quantizer_ep{epoch+1}.pt")
            
            # Save Best Model
            if avg_val_total < best_val_loss:
                best_val_loss = avg_val_total
                torch.save(quantizer.state_dict(), f"{log_dir}/best_quantizer.pt")
