...
class VCEncDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir, exc_file, avg_type, spk_ids=None):  # ✅ spk_ids 추가
        ...
        all_speakers = os.listdir(self.mel_x_dir)

        # ✅ 화자 필터링
        if spk_ids is not None:
            self.speakers = [spk for spk in all_speakers if spk in spk_ids]
        else:
...
class VCTKEncDataset_indv(torch.utils.data.Dataset):
    '''
    - The label of each mels file is the average mels 
    '''
    def __init__(self, data_dir, exc_file, avg_type, speaker):
        self.mel_x_dir = os.path.join(data_dir, 'mels')
        self.mel_y_dir = os.path.join(data_dir, 'mels_%s' % avg_type)
        ...
        mel_ids = os.listdir(os.path.join(self.mel_x_dir, speaker))
        mel_ids = [m for m in mel_ids if m.split('_')[1] not in self.unseen_sentences]
        mel_ids = [m[:-8] for m in mel_ids if m not in self.exceptions]
        #mel_ids = exclude_spn(data_dir, speaker, mel_ids)
        self.train_info += [(m, speaker) for m in mel_ids]


        mel_ids = os.listdir(os.path.join(self.mel_x_dir, speaker))
        mel_ids = [m for m in mel_ids if m.split('_')[1] in self.unseen_sentences]
        mel_ids = [m[:-8] for m in mel_ids if m not in self.exceptions]
        self.test_info += [(m, speaker) for m in mel_ids]
        print("Total number of test wavs is %d." % len(self.test_info))
        print("Total number of training wavs is %d." % len(self.train_info))
        random.seed(random_seed)
        random.shuffle(self.train_info)
...
class VCTKEncDataset_indv2indv(torch.utils.data.Dataset):
    '''
    - The label of each mels file is the average mels 
    '''
    def __init__(self, data_dir, exc_file, avg_type, speaker):
...
class VCTKDecDataset_indv(torch.utils.data.Dataset):
    def __init__(self, data_dir, exc_file, mode, spk):
        self.mel_dir = os.path.join(data_dir, 'mels')
        self.emb_dir = os.path.join(data_dir, 'embeds')
        self.unseen_sentences = get_vctk_unseen_sentences()
...
class VCTKDecDataset_cycle_indv(torch.utils.data.Dataset):
    def __init__(self, data_dir, exc_file, mode, spk):
        self.mel_dir = os.path.join(data_dir, 'mels')
        self.emb_dir = os.path.join(data_dir, 'embeds')
        self.unseen_sentences = get_vctk_unseen_sentences()
...
class VCDecBatchCollate_Cycle(object):
    def __call__(self, batch):
        B = len(batch)
        ...
        src_spks = [item['src_spk'] for item in batch] # 예: ['p236', 'p236', ...]
        tgt_spks = [item['tgt_spk'] for item in batch] # 예: ['p259', 'p263', ...]
        ...
        return {
            ...
            'c': embed, 
            'mel_tgt': mels_target, 
            'tgt_mel_lengths': mel_target_lengths, 
            'tgt_c': tgt_embed,
            'src_spk': src_spks, # <--- 추가됨 (List of str)
            'tgt_spk': tgt_spks  # <--- 추가됨 (List of str)
        }