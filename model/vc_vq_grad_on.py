...
from model.diffusion_grad_on import Diffusion
...
class DiffVC(BaseModule):
   ...
   # @torch.no_grad()
    def forward(self, x, x_lengths, x_ref, x_ref_lengths, c, n_timesteps, 
        ...
        z += torch.randn_like(mean_x_new, device=mean_x_new.device, requires_grad=False)
        y = self.decoder(z, x_mask_new, mean_new, x_ref, x_ref_mask, mean_ref, c, 
                         n_timesteps, mode)
    
        return mean_x, y[:, :, :max_length], mean, mean_ref

    #@torch.no_grad()
    def forward_vq(self, x, x_lengths, x_ref, x_ref_lengths, c,  
            mean_quantized, mean_ref_quantized, n_timesteps, mode='ml'):
        x, x_lengths = self.relocate_input([x, x_lengths])
        x_ref, x_ref_lengths, c = self.relocate_input([x_ref, x_ref_lengths, c])
        x_mask = sequence_mask(x_lengths).unsqueeze(1).to(x.dtype)
        x_ref_mask = sequence_mask(x_ref_lengths).unsqueeze(1).to(x_ref.dtype)
    ...
    def compute_loss(self, x, x_lengths, x_ref, c, quantizer):
        """
        ...
        x_mask = sequence_mask(x_lengths).unsqueeze(1).to(x.dtype)
        mean = self.encoder(x, x_mask)
        mean_new, _, _, q_loss, _ = quantizer(mean)

        mean_ref = self.encoder(x_ref, x_mask)
        mean_ref_new, _, _, q_loss_ref, _ = quantizer(mean_ref)

        diff_loss = self.decoder.compute_loss(x, x_mask, mean_new, x_ref, mean_ref_new, c)
        q_losses = q_loss + q_loss_ref
        return diff_loss, q_losses, mean_new, mean_ref_new
