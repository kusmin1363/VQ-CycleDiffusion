...
#                xt_ref += [self.compute_diffused_mean(ref, ref_mask, mean_ref, (j+0.5)/15.0)]
            ...
            if mode == 'pf':
                ...
                dxt += torch.randn_like(z, device=z.device, requires_grad=False) * sigma

            # print(f"[Step {i}] t={t:.20f} beta_t={beta_t:.20f}")
            # print(f"   xt mean={xt.mean().item():.20f}, std={xt.std().item():.20f}")
            # print(f"   dxt mean={dxt.mean().item():.20f}, std={dxt.std().item():.20f}")
            # print(f"   step max abs diff={dxt.abs().max().item():.20f}")
